...
const AddProducts = () => {
  ...
  return (
    <div className="flex-1 bg-main-bg">
      ...
      <div className="p-4">
        <h1 className="text-6xl font-bold text-center text-black mb-4 mt-2">
          <BsCartPlus className="inline-block mr-2" />Add Products</h1>
        <h3 className="text-lg md:text-base bg-main-green text-center text-white mt-4 md:mt-8 font-semibold">MAKE SURE PRODUCT ID IS UNIQUE</h3>
        ...
        <div className="flex justify-center items-center">
          <div className="flex flex-col items-start mt-2 bg-main-green overflow-hidden shadow-md p-6 rounded-xl w-4/5">
            
            <div className="flex flex-col w-full">
              <label className={`input-label ${productName ? 'active' : ''} font-semibold mb-2 text-white`}>Product Name</label>
              <input
                type="text"
                value={productName}
                onChange={(e) => setProductName(e.target.value)}
                className="py-2 px-4 rounded-md mb-2 shadow-md bg-white shadow-light-honey"
              />
            </div>
            <div className="flex flex-col w-full">
              <label className={`input-label ${description ? 'active' : ''} font-semibold mb-2 text-white`}>Description</label>
              <textarea
                value={description}
            ...
            <div className="flex flex-col w-1/3">
              <label className={`input-label font-semibold mb-2 text-white`}>Category</label>
              <select
            ...
            <div className="flex flex-col w-1/3">
              <label className={`input-label ${stockStatus ? 'active' : ''} font-semibold mb-2 text-white`}>Stock Status</label>
              <select
                value={stockStatus}
              ...
              {variations.map((variation, index) => (
                <div key={index} className="mb-6">
                  ...
                  <div className="flex flex-col w-1/2">
                    <label className={`input-label ${variation.temperature ? 'active' : ''} font-semibold mb-2 text-white`}>Temperature (hot/iced)</label>
                    <input
                      type="text"
                  ...
                  {variation.sizes.map((sizePrice, sizeIndex) => (
                    <div key={sizeIndex} className="flex mb-2 m-4 w-full items-center self-center">
                      {/* Size input */}
                      <div className="flex flex-col w-1/3">
                        <label className={`input-label ${sizePrice.size ? 'active' : ''} font-semibold mb-2 text-white`}>Size</label>
                        <input
                          type="text"
                          value={sizePrice.size}
                          onChange={(e) => handleSizePriceChange(index, sizeIndex, 'size', e.target.value)}
                          className="py-2 px-4 rounded-md mb-2 shadow-md bg-white shadow-light-honey mr-4"
                        />
                      </div>
                      {/* Price input */}
                      <div className="flex flex-col w-1/3">
                        <label className={`input-label ${sizePrice.price ? 'active' : ''} font-semibold mb-2 text-white`}>Price</label>
                        <input
                          type="number"
                  ...
                  <div className="flex justify-center">
                    <button onClick={() => handleAddSizePrice(index)} className="text-white shadow-md shadow-gray-500 bg-main-honey px-3 py-2 rounded-lg mt-6">Add Size & Price</button>
                  </div>
                </div>
              ))}
            </div>
